...
#This is used in final algorithm
...
class Node():
    ...
    con = [] # ["a", "b", "c"] can be in any order, might need to be a set
    ...
    def startNode(self):
        self.con = self.generateConstantsFromStart(problem) # Get from problem.constants
        self.literals = self.generateLiteralsFromStart(problem) # Get from problem.init
        self.children = self.generateChildrenFromActions(domain) #generate from actions.domain
    ...
    def generateChildrenFromActions(self, domain):
        children = []
        groundedLiterals = self.groundList(
            self.literals)  # TODO [["(p1 a b c)", "not(p2 b c)"], ["not(p2 b c)", "(p1 a b c)"]]
    ...
    def generatePredicatesFromAction(self, domain, con : list):
        param = [] #['?x', '?y', '?z'] This will be an issue when there is more than one action
        ...
        for action in domain.actions:
            ...
            for p in par:
                param.append(str(p))
            tup = action.precondition.operands
            for t in tup:
                pre.append(str(t))
